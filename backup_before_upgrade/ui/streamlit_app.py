...
st.caption(
    f"Retrieval engine: {health.get('retrieval_engine', 'hybrid-faiss-rerank-cited')} | "
...
if result and saved_payload:
    ...
    engine = result.get("retrieval_engine", "hybrid-faiss-rerank-cited")
    index_status = result.get("index_status", "unknown")
    st.caption(
        f"Draft mode: {mode} | Retrieval engine: {engine} | Index: {index_status}"
    )
